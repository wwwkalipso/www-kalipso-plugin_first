<!-- Ссылка ссылаеться на страницу гостевой книги только у нее добавлен $_GET['action'] параметр &action=add_data
 ...
 -->
<a href="admin.php?page=www_kalipso_control_guest_book_menu&action=add_data">
    <?php _e('Add', WWWKALIPSO_PlUGIN_TEXTDOMAIN ); ?>
</a>
...
<td>
    <!-- Ссылки  ссылаються на страницу гостевой книги только у них добавлен $_GET['action'] параметр
     ...
     действия -->
    <a href="admin.php?page=www_kalipso_control_guest_book_menu&action=edit_data&id=<?php echo $value['id'];?>">
        <?php _e('Edit', WWWKALIPSO_PlUGIN_TEXTDOMAIN ); ?>
    </a>
    <a href="admin.php?page=www_kalipso_control_guest_book_menu&action=delete_data&id=<?php echo $value['id'];?>">
